...
export default function TopNavbar() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/90 dark:bg-slate-950/90 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 md:px-6 h-20 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-6 shrink-0">
          ...
          >
            <img src="/ascendra-logo.png" alt="ASCENDRA" className="h-10 md:h-11 w-auto object-contain" />
          </button>
        </div>

        {/* Center: Curvy Top Navigation Links (Desktop) */}
        <nav className="hidden xl:flex items-center bg-slate-100/90 dark:bg-slate-900/90 p-1.5 rounded-full border border-slate-200/80 dark:border-slate-800 shadow-xs">
          {menuItems.map((item) => {
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          ...
          <div className="relative hidden lg:block w-56 xl:w-64">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 rounded-full bg-slate-100/80 dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 focus:outline-none focus:ring-2 focus:ring-black text-xs font-semibold text-black dark:text-white placeholder-slate-400"
            />
            {/* Search Dropdown */}
            {searchVal.trim().length > 0 && (
              <div className="absolute top-12 left-0 right-0 max-h-60 overflow-y-auto bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-[1.75rem] shadow-xl z-50 p-2 flex flex-col gap-1">
                {SEARCHABLE_ITEMS.filter(item =>
                  ...
                  <button
                    ...
                    className="w-full text-left p-3 rounded-2xl hover:bg-slate-50 dark:hover:bg-slate-800 flex flex-col gap-0.5 border border-transparent transition-all select-none"
                  >
                ...
                {SEARCHABLE_ITEMS.filter(item =>
                  ...
                  <span className="text-[10px] text-slate-400 font-bold text-center py-4 select-none">No matches found.</span>
                )}
              </div>
            )}
          </div>
          ...
          <button
            onClick={toggleTheme}
            className="p-2.5 rounded-full bg-slate-100/80 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200/80 dark:border-slate-800 transition-all"
            ...
            <button
              ...
              className={`p-2.5 rounded-full border transition-all flex items-center justify-center relative ${
                notiOpen 
                  ...
                  : 'bg-slate-100/80 dark:bg-slate-900 border-slate-200/80 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-800'
              }`}
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 p-1 pr-3 rounded-full border border-slate-200/80 dark:border-slate-800 bg-slate-100/80 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-800 transition-all shrink-0"
            >
              <div className="w-8 h-8 rounded-full bg-black text-white dark:bg-white dark:text-black flex items-center justify-center font-black text-xs">
          ...
          <button
            ...
            className="p-2.5 rounded-full bg-slate-100/80 dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 text-black dark:text-white xl:hidden"
        ...
        {mobileMenuOpen && (
          <>
            ...
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -20 }}
              className="fixed top-20 left-4 right-4 z-50 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-[2.5rem] p-6 shadow-2xl xl:hidden flex flex-col gap-4"
            >
