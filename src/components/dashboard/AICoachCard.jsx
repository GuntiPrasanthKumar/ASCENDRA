...
export default function AICoachCard({ title, type, description, aiInsight, matchScore, actionText, actionUrl, onAction }) {
  return (
    <div className="relative overflow-hidden p-8 rounded-[2.5rem] bg-white border border-slate-200/80 flex flex-col justify-between h-full group hover:border-slate-300 shadow-xs transition-all duration-300">
      ...
      <div>
        ...
        <div className="flex justify-between items-center mb-6">
          <span className="text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full bg-slate-100 border border-slate-200 text-black flex items-center gap-1">
            <Zap className="w-3.5 h-3.5 text-black" /> AI Coach
          </span>
          {matchScore && (
        ...
        <h3 className="text-xl font-bold font-display text-black mb-2 flex items-center gap-2">
          <BrainCircuit className="w-5 h-5 shrink-0 text-black" />
          {title}
        </h3>
        ...
        {aiInsight && (
          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/60 text-[11px] text-slate-700 leading-relaxed mb-6 font-medium">
