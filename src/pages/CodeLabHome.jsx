...
import { Code, ShieldAlert, Star, History, Filter, Sparkles, CheckCircle2, ChevronRight } from 'lucide-react';
    ...
    <PageTransition>
      <div className="min-h-screen bg-background pt-2 pb-12 px-4 md:px-6 relative overflow-hidden">
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="flex items-center justify-between mb-10 pb-6 border-b border-slate-200/80 flex-wrap gap-4">
            <div className="flex items-center gap-3.5">
              ...
              <div>
                <h1 className="text-3xl font-display font-medium text-slate-900 tracking-tight">CodeLab Studio</h1>
                <p className="text-slate-500 text-xs font-body mt-0.5">Monaco editor, multi-language compilation simulator, and AI code quality analysis.</p>
              </div>
            </div>
              ...
              <div>
                ...
                <div className="bg-white p-8 rounded-[1.75rem] border border-slate-200/80 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 shadow-xs">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 font-body leading-relaxed max-w-2xl">{continueProblem.description}</p>
                  </div>
                  <button
              ...
              <div className="lg:col-span-2 flex flex-col gap-8">
                  ...
                  <div>
                    ...
                    <div className="bg-white p-6 rounded-[1.75rem] border border-slate-200/80 flex items-center justify-between gap-4 shadow-xs">
                      <div className="flex gap-4 items-center">
                        ...
                        <div>
                          <h4 className="font-display font-medium text-slate-900 text-sm">{recommendedProblem.title}</h4>
                ...
                <div>
                  ...
                  <div className="bg-white p-4 rounded-[1.75rem] border border-slate-200/80 flex flex-wrap gap-4 items-center justify-between mb-6 shadow-xs">
                    ...
                    {/* Topic Pills */}
                    <div className="flex gap-1.5 flex-wrap items-center">
                      <span className="text-[10px] font-bold uppercase text-slate-400 mr-1">Topic:</span>
                      {allTopics.map((top) => (
                        <button
                          key={top}
                ...
                {dailyProblem && (
                  <div className="bg-white p-7 rounded-[1.75rem] border border-slate-200/80 flex flex-col justify-between h-full group hover:border-slate-300 shadow-xs transition-all duration-300">
                    <div>
                      ...
                      <h3 className="text-lg font-display font-medium text-slate-900 mb-2 tracking-tight">{dailyProblem.title}</h3>
                      <p className="text-xs text-slate-500 font-body leading-relaxed mb-6">{dailyProblem.description}</p>
                    </div>
                    <button
                  ...
                  <div className="flex flex-col gap-2.5">
                    {submissions.map((sub, i) => (
                      <div key={i} className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/60 text-xs text-slate-600 flex justify-between items-center">
                        <div>
                          <span className="font-extrabold text-[10px] text-slate-800 block mb-0.5">
                        ...
                        <div className="text-right">
                          ...
                          <span className="text-[9px] font-extrabold text-indigo-600">{sub.runtime}</span>
                        </div>
                      </div>
                    ))}
                  </div>
